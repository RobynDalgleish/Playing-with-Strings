...
function myFunction(){
        ...
        const removePunct = /[\u2000-\u206F\u2E00-\u2E7F\\'!"#$%&()*+,\-.\/:;<=>?@\[\]^_`{|}~]/g;
        const cleanStr = toLowercase.replace(removeSpaces, '').replace(removePunct, '');
        ...
        const sortedArray = splitStr.sort();
        ...
        const result = sortedArray.reduce(function (obj, item) {
            if (!obj[item]) {
                obj[item] = 0;
            }
            obj[item]++;
